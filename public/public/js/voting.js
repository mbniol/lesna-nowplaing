...
const scriptBlockDialog = document.querySelector(".script-block-dialog");
const voteBlockDialog = document.querySelector(".vote-block-dialog");
const voteBlockDialogButton = document.querySelector(
  ".vote-block-dialog__button"
);

const script = document.createElement("script");
script.src = `/js/fingerprint.js`;
script.onerror = (e) => {
  scriptBlockDialog.showModal();
};
script.onload = show_votes;
document.querySelector("head").append(script);

voteBlockDialogButton.addEventListener("click", () => {
  voteBlockDialog.close();
});
...
async function show_votes() {
  ...
  console.log(json);
  json.forEach(({ id, name, artist, votesCount, cover }) => {
  ...
  events.onmessage = async (event) => {
    const { trackID, cover, name, artist, votes } = JSON.parse(event.data);
    // console.log(JSON.stringify)
    console.log(JSON.parse(event.data));
    const targetElement = document.querySelector("#letterstart" + trackID);
    if (!targetElement) {
